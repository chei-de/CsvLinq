...
class TestCsvLinqTrims(unittest.TestCase):
    ...
    def setUp(self):
        self.csv_path = Path(r"Tests\cities.csv").absolute()

    def test_identity_trim(self):
        csv = CsvLinq(self.csv_path, trims.Identity())
    ...
    def test_removal_trim(self):
        csv = CsvLinq(self.csv_path, trims.StripWhitespaces(), trims.RemoveChars('"', "Y"))
